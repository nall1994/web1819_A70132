...
app.get('/ficheiros',(req,res)=> {
    jsonfile.readFile(catalog,(erro,catalogo)=> {
        if(!erro) {
            res.write(pug.renderFile('catalogo_ficheiros.pug',{ficheiros: catalogo}))
            res.end()
        } else {
        ...
        form.parse(req, (erro,fields,files)=> {
            ...
            fs.rename(fich_enviado,fich_novo, erro => {
                if(!erro) {
                    ...
                    res.write(pug.renderFile('ficheiro_recebido.pug',{ficheiro: files.ficheiro.name, estado: 'Ficheiro recebido com sucesso!'}))
                    res.end()
                } else {
